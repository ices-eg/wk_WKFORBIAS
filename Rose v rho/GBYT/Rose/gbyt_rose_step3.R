...
use12 <- read.csv("saved\\use12table.csv", header = TRUE)
use12
...
for (i in 1:nscen){
  if (use12$mselxlab[i] == "All Ages"){
    myselvec <- "111111"
  } else if (use12$mselxlab[i] == "Old"){
    myselvec <- "000111"
  } else if (use12$mselxlab[i] == "Young"){
    myselvec <- "111000"
  }
  fname <- paste0("y", use12$change.year[i], "r", use12$ramp[i], "c", use12$cmult[i],
                  "m", use12$mmult[i], "s", myselvec, "_000.rdat")
...
# plots
myorder <- as.character(use12$scen)
myorder[1:9] <- paste0("0", myorder[1:9])
scen.status.df <- data.frame(Scenario = rep(paste(myorder, use12$scenario, sep="_"), 3),
...
scen.c.df <- data.frame(Scenario = rep(paste(myorder, use12$scenario, sep="_"), 3),
